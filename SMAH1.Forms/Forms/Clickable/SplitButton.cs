...
// SMAH1 : 
//      2012/07/11 : Add Right to left support
...
namespace SMAH1.Forms.Clickable
{
    ...
    public partial class SplitButton : Button
    {
        ...
        private void InitDefaultSplitImages()
        {
            if (this.IsRTL())
            {
                _NormalImage = "Normal2";
                _HoverImage = "Hover2";
                _ClickedImage = "Clicked2";
                _DisabledImage = "Disabled2";
                _FocusedImage = "Hover2";
                ImageKey = "Normal2";
            }
            else
            {
        ...
        private void InitDefaultSplitImages(bool refresh)
        {
            if (this.IsRTL())
            {
                if (string.IsNullOrEmpty(_NormalImage)) _NormalImage = "Normal2";
                if (string.IsNullOrEmpty(_HoverImage)) _HoverImage = "Hover2";
                if (string.IsNullOrEmpty(_ClickedImage)) _ClickedImage = "Clicked2";
                if (string.IsNullOrEmpty(_DisabledImage)) _DisabledImage = "Disabled2";
                if (string.IsNullOrEmpty(_FocusedImage)) _FocusedImage = "Hover2";
            }
            else
            {
        ...
        private Rectangle GetImageRect(string imageKey)
        {
            ...
            Rectangle rcRet = new Rectangle(x, y, w, h);
            if (this.IsRTL())
                rcRet = new Rectangle(0, 0, w + 1, h + 1);
        ...
        private void EventFire(EventHandler evntHndlr, EventArgs ea)
        {
            ...
            foreach (Delegate del in evntHndlr.GetInvocationList())
            {
                try
                {
                    ...
                    //
                    if (!(del.Target is ISynchronizeInvoke syncr))
                    {
                        //
        ...
        {
            const int TOO_CLOSE_IN_MILLISECONDS = 150;
                ...
                if (_AlwaysDropDown || _AlwaysHoverChange || IsMouseInSplit())
                {
                    ...
                    if (ContextMenuStrip != null && ContextMenuStrip.Items.Count > 0)
                    {
                        ...
                        if (!IsTooSoonAfterCloseMenuDropDown())
                            ContextMenuStrip.Show(this, new Point(x, Height));
                    }
                }
                else
                {
